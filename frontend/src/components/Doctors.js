...
const Doctors = () => {
  ...
  const handleAddDoctor = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8000/doctors/add", newDoctor)
      .then((response) => {
  ...
  const handleUpdateDoctor = (id, e) => {
    e.preventDefault();
    axios
      .post(`http://localhost:8000/doctors/update/${id}`, selectedDoctor)
      .then((response) => {
        const updateDoc = {
          ...selectedDoctor,
          _id: id,
  ...
  const handleDeleteDoctor = (id) => {
    axios
      .delete(`http://localhost:8000/doctors/delete/${id}`)
      .then((response) => {
    ...
    <div className="main-doc-container">
      <div className="form-sections">
        ...
        >
          ...
          <br />
          <button type="submit">
            {isEditMode ? "UpdateDoctor" : "Add Doctor"}
          </button>
        </form>
      </div>
